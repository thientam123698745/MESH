...
#include "nvs_flash.h"
#include "mesh_app.h"
static const char *MESH_TAG = "NODE_SCAN_DEVICE";
mesh_route_t device_node;
void scan_device_route()
{
    int node_num = 0;
    while (true)
    {
        // ESP_LOGI(MESH_TAG, "Dang scan device");
        esp_mesh_get_routing_table((mesh_addr_t *)&device_node.route_table, 50 * num_ap_connected, &device_node.route_table_size);
